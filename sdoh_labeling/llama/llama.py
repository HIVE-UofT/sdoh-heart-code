...
"""
Run zero-shot SDoH classification with Llama-3 Instruct.
"""
...
def generate_labels(model, tokenizer, comments, batch_size: int, max_new_tokens: int, device):
    ...
    for i in tqdm(range(0, len(comments), batch_size), desc="Batches", leave=False):
        ...
        # Include Llama's end-of-turn token if present, for clean stopping.
        terminators = [tokenizer.eos_token_id]
        try:
            eot_id = tokenizer.convert_tokens_to_ids("<|eot_id|>")
            if isinstance(eot_id, int) and eot_id != tokenizer.eos_token_id:
...
def main():
    ...
    for start in range(start_idx - (start_idx % chunk_size), n, chunk_size):
        ...
        print(f"[llama3] Saved rows {start}..{end-1} -> {args.output}")

    # Final save
    out_df.to_csv(args.output, index=False)
    print(f"[llama3] Done. Wrote: {args.output}")

if __name__ == "__main__":
    input_path = "/home/minhle/projects/aip-btaati/minhle/Test/2023-08-01 - Pivot of comments by valence - MBC_Comments.csv"
    output_path = "/home/minhle/projects/aip-btaati/minhle/Test/llama/test_llama3.csv"
    text_col = "Comment"
